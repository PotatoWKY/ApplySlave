...
from hamster.job_discovery.sources.base import ATSSource
...
def load_default_companies() -> dict[str, list[str]]:
    ...
    resource = resources.files("hamster.job_discovery").joinpath("companies.yaml")
...
def build_default_aggregator(
...
) -> tuple[DiscoveryAggregator, list[ATSSource | JSearchSource]]:
    ...
    Every source the aggregator runs is in the returned list, so closing all of
    them releases every client this function created.
    ...
    sources: list[ATSSource | JSearchSource] = []
    ...
    # same JobSource protocol, so the aggregator treats it uniformly and it
    # joins the same list — letting the caller close its HTTP client too.
    if jsearch_api_key:
        sources.append(JSearchSource(api_key=jsearch_api_key, client=client))

    return DiscoveryAggregator(sources=sources), sources
