...
//!
//! In dev mode we invoke `uv run hamster-backend` from the workspace root,
...
fn workspace_root() -> PathBuf {
    ...
    // src-tauri -> hamster-desktop -> apps -> repo root
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
fn sidecar_binary_path() -> PathBuf {
    ...
    let candidate = exe_dir.join("hamster-backend");
    if candidate.exists() {
        return candidate;
    }
    ...
    let resources = exe_dir
        .parent()
        .map(|p| p.join("Resources").join("hamster-backend"));
