...
function WildModeSection() {
  ...
  const updateMutation = useMutation({
    mutationFn: (wild: boolean) =>
      backendClient.saveSettings({ wild_mode: wild ? "true" : "" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
    },
  });

  // Default: wild mode is off unless explicitly set
  const isOn = Boolean(settingsQuery.data?.wild_mode);

  return (
    <section className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-medium">Wild mode</h2>
          <p className="mt-1 text-sm text-slate-500">
            When OFF, the pipeline only submits when it's confident every
            field is filled correctly — otherwise it stops for your review.
            When ON, it submits no matter what, so you never have to touch
            anything. Only relevant when dry-run is off.
          </p>
        </div>
        <button
          type="button"
      ...
      {isOn && (
        <div className="mt-3 rounded-md bg-amber-50 p-3 text-sm text-amber-700">
          <strong>Wild mode is on.</strong> Forms will be submitted even when
          the AI isn't sure it filled every field correctly.
        </div>
      )}
    </section>
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-xs text-slate-500">
            Screenshots saved to ~/Library/Application Support/Hamster/screenshots/
          </p>
        </div>
        <button
          type="button"
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 flex items-center gap-3 text-xs text-slate-400">
        <span>Model stored at ~/Library/Application Support/Hamster/models/</span>
